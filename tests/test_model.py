...
def test_model():
    ...
    model = CNNModel()
    assert model.forward(input.float()).shape == (input.shape[0],10), f'Output format of model is not ({input.shape[0]},10)'
...
def test_channels():
    with pytest.raises(ValueError, match='Expected channel size not equal to 1'):
       model = CNNModel()
